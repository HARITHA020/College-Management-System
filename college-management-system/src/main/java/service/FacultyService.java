...
// ✅ IMPORTANT CHANGE HERE
	...
	public void viewTimetable(int facultyId) {
		...
		for (Timetable t : list) {
			...
			if (t.getFacultyId() == facultyId) {
				...
				System.out.println(t.getDay() + " " + t.getTime() + " → Course ID: " + t.getCourseId() + " → Class: "
						+ t.getSection() + " → Room: " + t.getRoom());
	...
	public void uploadMaterial(int courseId, String title, String content, int facultyId) {
		...
		for (Material m : materialDAO.getAllMaterials()) {
			if (m.getCourseId() == courseId && m.getTitle().equalsIgnoreCase(title.trim())) {
		...
		// 🔹 9. Create & save
		Material m = new Material(newId, courseId, title.trim(), content.trim());
	...
	public void viewMaterials(int courseId, int userId, String role) {
		...
		if (role.equalsIgnoreCase("STUDENT")) {
			...
			for (Enrollment e : enrollmentDao.getAllEnrollments()) {
				if (e.getStudentId() == userId && e.getCourseId() == courseId) {
					enrolled = true;
					break;
				}
			}
	...
	public void createAssignment(int courseId, String title, String description, int facultyId) {
		...
		for (Assignment a : assignmentDAO.getAllAssignments()) {
			if (a.getCourseId() == courseId && a.getTitle().equalsIgnoreCase(title.trim())) {
		...
		// 🔹 9. Create & store
		Assignment a = new Assignment(newId, courseId, title.trim(), description.trim());
